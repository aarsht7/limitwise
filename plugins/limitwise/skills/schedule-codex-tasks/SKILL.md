...
10. A task may explicitly continue an existing `quota_interrupted` or `quota_skipped` task. Verify it with `get_task_status`, require the same project worktree, show its trigger as `after TASK_ID's five-hour reset`, pass `continue_from_task_id`, and omit `run_at` and `after_previous`. Never offer ordinary failed, blocked, cancelled, missed, completed, scheduled, or running tasks as continuation sources. Never create a continuation automatically.
11. Show one confirmation table with task, plan, success criteria, difficulty, model, effort, exact timestamp/timezone or dependency trigger, project, permissions, shared budget mode/cap, optional five-hour cap, estimate range, and confidence.
12. Do not call `schedule_batch`, `setup_service`, `update_task`, or `cancel_task` in Plan mode. Ask the user to confirm the table and switch to normal mode.
